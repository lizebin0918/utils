...
package com.base.framework.common.log;
...
public class DailyMaxRollingFileAppender extends FileAppender {
	 ...
	 */
	private volatile long nextCheck = System.currentTimeMillis() - 1;
	...
	public void activateOptions() {
		...
		LogLog.debug("Max backup file size kept: " + MaxFileSize + ".");
	...
	void rollOver() throws IOException {
		...
		String datedFilename = fileName + sdf.format(_now);
		// It is too early to roll over because we are still within the
		// bounds of the current interval. Rollover will occur once the
		// next interval is reached.
		if (scheduledFilename.equals(datedFilename) && isNeedBackup == false) {
			return;
		}

		SimpleDateFormat sdf2 = new SimpleDateFormat("HH-mm-ss");

		String time = sdf2.format(_now);
		// close current file, and rename it to datedFilename
		this.closeFile();

		File target = new File(scheduledFilename + "-" + time);
		if (target.exists()) {
			target.delete();
		}
	...
	protected void subAppend(LoggingEvent event) {
		...
		File file = new File(fileName);
		if (n >= nextCheck) {
			nextCheck = rpc.getNextCheckMillis(new Date());
		...
		} else if (file.exists() && file.length() >= MaxFileSize) {
			try {
...
class MaxRollingCalendar extends GregorianCalendar {
	...
	MaxRollingCalendar(TimeZone tz, Locale locale) {
		super(tz, locale);
	}
 ...
 */
@SuppressWarnings("serial")
class RollingPastCalendar extends MaxRollingCalendar {
	RollingPastCalendar() {
		super();
	}
