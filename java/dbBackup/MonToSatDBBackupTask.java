 ...
 *
 */
public class MonToSatDBBackupTask {
	...
	public void DBBackup(){
		...
		try{
			...
			for(DBBackupOption dBBackupOption : dbBackupOptions){
				String weekday=TimeUtil.getWeekday(TimeUtil.getDateRelateToDate(new Date(), -1));
				String date=TimeUtil.dateFormat(TimeUtil.getDateRelateToDate(new Date(), -1), "yyyyMMdd");
				...
				backupFileName +="."+weekday+"_" + TimeUtil.getCurDate("yyyyMMddHHmmss");
				/*1.备份数据库*/
				DBBackupSqlTool.exportDatabase(backupFileName, conn, isTableStructure, tableNames,date);
